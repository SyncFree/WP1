...
MergePNCounters(pnc1, pnc2) == [p |-> [d \in DOMAIN pnc1.p |-> Max(pnc1.p[d], pnc2.p[d])], 
...
\* ---DOES NOT HOLD
ConservationOfMoney == \A r \in Replicas: 
	EvalPNCounter(wallets[r].balance)  + EvalPNCounter(wallets[r].v1cnt)*V1Cost = 0
...
EqualStates(st1, st2) == \A rep \in Replicas: 
	st1.balance.p[rep] = st2.balance.p[rep] /\ 
...
Convergence == \A r1 \in Replicas, r2 \in Replicas: 
	FinalState(wallets[r1].vecclc) /\ FinalState(wallets[r2].vecclc) => 
		EqualStates(wallets[r1], wallets[r2])

Next == \E r1 \in Replicas, r2 \in Replicas, qty \in 1..Qtylim: 
	(BuyV1(r1,qty) \/ MergeCount(r1,r2) \/ MergeBalance(r1,r2)) 
