...
\fl{}\moduleLeftDash\cl{ {\MODULE} adCounterState\_v0}\moduleRightDash\cl{}
\fl{ {\EXTENDS} Naturals}
\fl{}
\fl{ {\CONSTANTS}}
\fl{ DC ,\,}\al{4}{2}{%
...
\fl{ maxTotalViews ,\,%
\@y{\@s{0}%
 Maximum number of times an ad should be shown.
}%
}
\fl{ maxTotalViewsPerDC ,\,}\al{8}{2}{%
\@y{\@s{0}%
 Maximum number of times an ad should be shown in a data center.
}%
}
\fl{}\al{9}{0}{}
\fl{ maxTotalViewsPerDevice ,\,%
\@y{\@s{0}%
 Maximum number of times an ad should be shown in a device.
}%
}
\fl{ deviceAssignment}\al{11}{1}{%
\@y{\@s{0}%
...
\fl{}\al{12}{0}{}
\fl{}
\fl{ Max ( x ,\, y ) \.{\defeq} {\IF} x \.{>} y \.{\THEN} x \.{\ELSE} y}
\fl{}
\fl{ SumAll ( func ) \.{\defeq}}
\fl{ \.{\LET}}\al{17}{1}{ sum [ p \.{\in} func ] \.{\defeq}}
 \fl{ {\IF} func \.{\,\backslash\,} \{ p \} \.{=} \{ \} \.{\THEN} p [ 2 ]
 \.{\ELSE} p [ 2 ] \.{+} sum [ func \.{\,\backslash\,} \{ p \} ]}
\fl{ \.{\IN} sum [ func ]}
\fl{}
\fl{ {\ASSUME} maxTotalViews \.{\in} [ AD \.{\rightarrow} Nat ]}
 \fl{ {\ASSUME} maxTotalViewsPerDevice \.{\in} [ AD \.{\rightarrow} [ DV
 \.{\rightarrow} Nat ] ]}
\fl{ {\ASSUME} deviceAssignment \.{\in} [ DV \.{\rightarrow} DC ]}
 \fl{ {\ASSUME} maxTotalViewsPerDC \.{\in} [ DC \.{\rightarrow} [ AD
 \.{\rightarrow} Nat ] ]}
\fl{}
\fl{}
\fl{ {\VARIABLES} configuration}
\fl{}
\fl{}\midbar\cl{}
\fl{}
\fl{ GCounter \.{\defeq} [ DC \.{\rightarrow} Nat ]}
\fl{}
\fl{}
\fl{}
\fl{}
\fl{}
\fl{ State \.{\defeq}}
\fl{ [}\al{38}{1}{ ads \.{:} {\SUBSET} AD ,\,%
\@y{\@s{0}%
 Set of devices assigned to the shown on that data center
}%
}
\fl{ devices \.{:} {\SUBSET} DV ,\,%
\@y{\@s{0}%
 Set of devices assigned to the data center.
}%
}
\fl{}
\fl{ maxViews \.{:} [ AD \.{\rightarrow} Nat ] ,\,}\al{41}{8}{%
\@y{\@s{0}%
 Maximum number of times an ad should be shown in
}%
}
\fl{}\al{42}{0}{}
\fl{}
\fl{ views \.{:} [ AD \.{\rightarrow} GCounter ] ,\,%
\@y{\@s{0}%
 G-Counter with overall views for each ad.
}%
}
\fl{}
 \fl{ viewsPerDevice \.{:} [ AD \.{\rightarrow} [ DV \.{\rightarrow} Nat ] ]
 ]}\al{46}{12}{%
\@y{\@s{0}%
 Number of views of each ad by a device.
}%
}
\fl{}\al{47}{0}{}
\fl{}\al{48}{0}{}
\fl{}\al{49}{0}{}
\fl{}
 \fl{ TypeInvariant \.{\defeq} configuration \.{\in} [ DC \.{\rightarrow}
 State ]}
\fl{}
\fl{ Init \.{\defeq}}
\fl{ \.{\land}}\al{54}{1}{ TypeInvariant}
\fl{ \.{\land}}\al{55}{1}{ configuration \.{=}}
 \fl{ [ d \.{\in} DC \.{\mapsto} [}\al{56}{6}{ ads \.{\mapsto} \{ a \.{\in} AD
 \.{:} maxTotalViewsPerDC [ d ] [ a ] \.{>} 0 \} ,\,}
 \fl{ devices \.{\mapsto} \{ g \.{\in} DV \.{:} deviceAssignment [ g ] \.{=} d
 \} ,\,}
 \fl{ maxViews \.{\mapsto} [ a \.{\in} AD \.{\mapsto} maxTotalViewsPerDC [ d ]
 [ a ] ] ,\,}
 \fl{ views \.{\mapsto} [ a \.{\in} AD \.{\mapsto} [ d1 \.{\in} DC \.{\mapsto}
 0 ] ] ,\,}
 \fl{ viewsPerDevice \.{\mapsto} [ a \.{\in} AD \.{\mapsto} [ g \.{\in} DV
 \.{\mapsto} 0 ] ] ] ]}
\fl{}
\fl{}\midbar\cl{}
\fl{}
...
\fl{}
\fl{}
\fl{ Inc (}\al{71}{2}{ d ,\, a ,\, g ) \.{\defeq}}
\fl{ \.{\LET}}\al{72}{1}{ state \.{\defeq} configuration [ d ]}
\fl{ gc \.{\defeq} state . views [ a ]}
\fl{ new\_state \.{\defeq}}
\fl{ [}\al{75}{1}{ ads \.{\mapsto} state . ads ,\,}
\fl{ devices \.{\mapsto} state . devices ,\,}
\fl{ maxViews \.{\mapsto} state . maxViews ,\,}
 \fl{ views \.{\mapsto} [ state . views {\EXCEPT} {\bang} [ a ] [ d ] \.{=} @
 \.{+} 1 ] ,\,}
 \fl{ viewsPerDevice \.{\mapsto} [ state . viewsPerDevice {\EXCEPT} {\bang} [
 a ] [ g ] \.{=} @ \.{+} 1 ] ]}
 \fl{ \.{\IN}}\al{80}{1}{ \.{\land}}\al{80}{2}{ a}\al{80}{3}{
 \.{\in}}\al{80}{4}{ state . ads}
\fl{ \.{\land}}\al{81}{1}{ g}\al{81}{2}{ \.{\in}}\al{81}{3}{ state . devices}
\fl{ \.{\land}}\al{82}{1}{ gc [ d ] \.{<} state . maxViews [ a ]}
 \fl{ \.{\land}}\al{83}{1}{ state . viewsPerDevice [ a ] [ g ] \.{<}
 maxTotalViewsPerDevice [ a ] [ g ]}
 \fl{ \.{\land}}\al{84}{1}{ configuration \.{'} \.{=} [ configuration
 {\EXCEPT} {\bang} [ d ] \.{=} new\_state ]}
\fl{}
\fl{ Merge ( d1 ,\, d2 ) \.{\defeq}}
 \fl{ \.{\LET}}\al{87}{1}{ state1}\al{87}{2}{ \.{\defeq}}\al{87}{3}{
 configuration [ d1 ]}
\fl{ state2}\al{88}{1}{ \.{\defeq}}\al{88}{2}{ configuration [ d2 ]}
\fl{ new\_state1 \.{\defeq}}
\fl{ [}\al{90}{1}{ ads \.{\mapsto} state1 . ads ,\,}
\fl{ devices \.{\mapsto} state1 . devices ,\,}
\fl{ maxViews \.{\mapsto} state1 . maxViews ,\,}
 \fl{ views}\al{93}{1}{ \.{\mapsto} [ a \.{\in} AD \.{\mapsto} [ d \.{\in} DC
 \.{\mapsto}}
\fl{ {\IF} a \.{\in} state1 . ads}
 \fl{ \.{\THEN} Max ( state1 . views [ a ] [ d ] ,\, state2 . views [ a ] [ d
 ] )}
\fl{ \.{\ELSE} 0 ] ] ,\,}
\fl{ viewsPerDevice \.{\mapsto} state1 . viewsPerDevice ]}
 \fl{ \.{\IN} configuration \.{'} \.{=} [ configuration {\EXCEPT} {\bang} [ d1
 ] \.{=} new\_state1 ]}
\fl{}
\fl{}
 \fl{ Views ( d ,\, a ) \.{\defeq} SumAll ( configuration [ d ] . views [ a ]
 )}
\fl{}
\fl{}\midbar\cl{}
\fl{}
\fl{ Consistency \.{\defeq}}
\fl{}
 \fl{ \.{\land} \A\, d \.{\in} DC ,\, a \.{\in} AD \.{:} Views ( d ,\, a )
 \.{\leq} maxTotalViews [ a ]}
\fl{}
\fl{}
 \fl{ \.{\land} \A\, d \.{\in} DC ,\, a \.{\in} AD \.{:} configuration [ d ] .
 views [ a ] [ d ] \.{\leq} maxTotalViewsPerDC [ a ] [ d ]}
\fl{}
\fl{}
 \fl{ \.{\land} \A\, d \.{\in} DC ,\, a \.{\in} AD \.{:} a \.{\notin}
 configuration [ d ] . ads \.{\implies} Views ( d ,\, a ) \.{=} 0}
\fl{}
\fl{}
 \fl{ \.{\land} \A\, d}\al{116}{3}{ \.{\in} DC ,\, a \.{\in} AD ,\, g \.{\in}
 DV \.{:}}
 \fl{ configuration [ d ] . viewsPerDevice [ a ] [ g ] \.{\leq}
 maxTotalViewsPerDevice [ a ] [ g ]}
\fl{}
\fl{}
 \fl{ \.{\land}}\al{120}{1}{ \A\, d \.{\in} DC ,\, a \.{\in} AD ,\, g \.{\in}
 DV \.{:}}
 \fl{ \.{\land}}\al{121}{1}{ g}\al{121}{2}{ \.{\notin}}\al{121}{3}{
 configuration [ d ] . devices \.{\implies} configuration [ d ] .
 viewsPerDevice [ a ] [ g ] \.{=} 0}
 \fl{ \.{\land}}\al{122}{1}{ a}\al{122}{2}{ \.{\notin}}\al{122}{3}{
 configuration [ d ] . ads \.{\implies} configuration [ d ] . viewsPerDevice
 [ a ] [ g ] \.{=} 0}
\fl{}
\fl{}
 \fl{ \.{\land} \A\, d \.{\in} DC ,\, a \.{\in} AD \.{:} SumAll (
 configuration [ d ] . viewsPerDevice [ a ] ) \.{=} Views ( d ,\, a )}
\fl{}
\fl{}
\fl{}
 \fl{ \.{\land} \A\,}\al{129}{2}{ d1 \.{\in} DC ,\, d2 \.{\in} DC ,\, a
 \.{\in} AD \.{:}}
 \fl{ configuration [ d1 ] . views [ a ] [ d1 ] \.{\geq} configuration [ d2 ]
 . views [ a ] [ d1 ]}
\fl{}
 \fl{ Next}\al{132}{1}{ \.{\defeq}}\al{132}{2}{ \E\, d1 \.{\in} DC ,\, d2
 \.{\in} DC ,\, a \.{\in} AD ,\, g \.{\in} DV \.{:} Inc ( d1 ,\, a ,\, g )
 \.{\lor} Merge ( d1 ,\, d2 )}
\fl{}
 \fl{ vars}\al{134}{1}{ \.{\defeq}}\al{134}{2}{ {\langle} configuration
 {\rangle}}
\fl{}\midbar\cl{}
\fl{ Spec \.{\defeq} Init \.{\land} {\Box} [ Next ]_{ vars}}
\fl{}\midbar\cl{}
\fl{ {\THEOREM} Spec \.{\implies} TypeInvariant \.{\land} Consistency}
\fl{}
\fl{}\bottombar\cl{}
\fl{}
\fl{}
