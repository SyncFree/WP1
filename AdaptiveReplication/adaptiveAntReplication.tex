...
\subsection{Mathematical Cost Representation}
...
\begin{table}[ht!]
	\begin{center}
		\begin{tabular}{|l|l|l|}
			...
			\hline
			$c^{r}_{kdj}$  & The cost of a read request of data $k$ from \gls{dc} $d$ to \gls{dc} $j$. & $\mathbb{R}^{+}$ \\
			\hline
			$c^{r}_{*dj}$  & The cost of a search request from \gls{dc} $d$ to \gls{dc} $j$. & $\mathbb{R}^{+}$ \\
			\hline
			$c^{w}_{kdj}$ & The cost of a read request of data $k$ from \gls{dc} $d$ to \gls{dc} $j$. & $\mathbb{R}^{+}$ \\
			\hline
		\end{tabular}
	\end{center}
...
The extended cost is represented in Equation \ref{eq:full_cost_operations}.
